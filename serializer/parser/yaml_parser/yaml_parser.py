from serializer.dto.dto import DTO, DTO_TYPE
from types import NoneType, FunctionType, ModuleType, CodeType
import yaml
import imp
...
class YAML_Parser():
    ...
    def _parse_types(self, obj: any) -> any:
        ...
        if type(obj) in prim_types:
            res_obj = self._parse_prim_type(obj)
        elif type(obj) == list:
            res_obj = self._parse_list_type(obj)
        else:
            res_obj = self._parse_dto(obj)
        return res_obj
    ...
    def _parse_list_type(self, list_obj: list) -> list:
        res_list = []
        for item in list_obj:
            res_list.append(self._parse_types(item))
        return res_list


    def _parse_dto(self, dto_obj: any) -> any:
        if dto_obj[DTO.dto_type] == DTO_TYPE.dict:
    ...
    def _parse_dict_type(self, dict_obj: dict) -> dict:
        ...
        for item in dict_obj.items():
            res_dict[self._parse_types(item[0])] = self._parse_types(item[1])
        return res_dict
    ...
    def _parse_func_type(self, func_obj: dict) -> FunctionType:
        ...
        func_code = CodeType(
            ...
            code_dict["co_code"], tuple(code_dict["co_consts"]),
